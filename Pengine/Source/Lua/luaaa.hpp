...
#ifndef LUAAA_WITHOUT_CPP_STDLIB
            luaL_argcheck(state, true, 1, (std::string("C++ class `") + RTTI_CLASS_NAME(TCLASS) + "` bind to conflict lua name `" + name + "`, origin name: " + klassName).c_str());
#else
        ...
        inline LuaClass<TCLASS>& ctor(const char * name = "new")
        {
            struct HelperClass {
                static int f_gc(lua_State* state) {
                    if (LuaClass<TCLASS>::klassName && lua_isuserdata(state, -1)) {
        ...
        inline LuaClass<TCLASS>& ctor(const char * name, TCLASS*(*spawner)(ARGS...), TRET(*deleter)(TCLASS*)){
            ...
            struct HelperClass {
                static int f_gc(lua_State* state) {
                    ...
                    if (LuaClass<TCLASS>::klassName && deleter) {
